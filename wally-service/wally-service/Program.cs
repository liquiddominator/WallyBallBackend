...
using Serilog;
using WallyBallBackend.Api.OpenApi;
using WallyBallBackend.Application;
using WallyBallBackend.Infrastructure;
...
try
{
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "WallyBallBackend API",
            Version = "v1",
            Description = "API para la gestion de campeonatos de wallyball del Deportivo Agape."
        });
    ...
    if (app.Environment.IsDevelopment() || app.Environment.IsEnvironment("Docker"))
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "WallyBallBackend API v1");
            options.RoutePrefix = "swagger";
            options.DocumentTitle = "WallyBallBackend API";
        });
    }
    ...
    app.Lifetime.ApplicationStarted.Register(() =>
    {
        ...
        {
            Log.Information("API available at {Url}", url);
...
static async Task ApplyDatabaseMigrationsAsync(WebApplication app)
{
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        try
        {
            ...
            Log.Information("Applying SQL Server migrations. Attempt {Attempt}/{MaxAttempts}", attempt, maxAttempts);
        ...
        catch (Exception exception) when (attempt < maxAttempts)
        {
            Log.Warning(
                exception,
                "SQL Server migration attempt {Attempt}/{MaxAttempts} failed. Retrying in {DelaySeconds} seconds.",
                attempt,
                maxAttempts,
                delay.TotalSeconds);
    ...
    Log.Information("Applying SQL Server migrations. Final attempt {MaxAttempts}/{MaxAttempts}", maxAttempts);
